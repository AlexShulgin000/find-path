 ...
 * */
export const getHeroAllowedSteps = (blocks: TBlocks, hPosition: IHeroPosition) => {
    ...
    for (let celIndex = 0; celIndex < row.length; celIndex++) {
        ...
        if(hPosition.rowIndex === currentRowIndex && (!cellNumber || (currentStepNumber && cellNumber > currentStepNumber))){
            if (
